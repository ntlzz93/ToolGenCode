...
namespace BussinessLogic
{
    public  class |@Table@|BO
    {
		...
		public List<|@Table@|> Sel_ByIDLang( int IDLang)
        {
            try
            {
                return aDatabaseDA.|@Table@|.Where(z => z.IDLang == IDLang).ToList();
            }
            catch (Exception ex)
            {
                throw new Exception(String.Format("|@Table@|BO.Sel_ByIDLang:{0}", ex.Message));
            }
        }
		public List<|@Table@|> Sel_ByIDLang( int IDLang, bool Disable)
        {
            try
            {
                return aDatabaseDA.|@Table@|.Where(z => z.IDLang == IDLang).Where(z => z.Disable == Disable).ToList();
            }
            catch (Exception ex)
            {
                throw new Exception(String.Format("|@Table@|BO.Sel_ByIDLang:{0}", ex.Message));
        ...
        public List<|@Table@|> Sel_ByType_ByIDLang(int Type, int IDLang)
        {
            try
            {
                return aDatabaseDA.|@Table@|.Where(p => p.Type == Type).Where(z => z.IDLang == IDLang).ToList();
            }
            catch (Exception ex)
            {
        ...
        public List<|@Table@|> Sel_ByType_ByIDLang(int Type, int IDLang, bool Disable)
        {
            try
            {
                return aDatabaseDA.|@Table@|.Where(p => p.Type == Type).Where(z => z.IDLang == IDLang).Where(z => z.Disable == Disable).ToList();
            }
            catch (Exception ex)
            {
                throw new Exception(String.Format("|@Table@|BO.Sel_ByType:{0}", ex.Message));
        ...
        public List<|@Table@|> Sel_ByStatus_ByIDLang(int Status, int IDLang)
        {
            try
            {
                return aDatabaseDA.|@Table@|.Where(p => p.Status == Status).Where(z => z.IDLang == IDLang).ToList();
            }
            catch (Exception ex)
            {
        ...
        public List<|@Table@|> Sel_ByStatus_ByIDLang(int Status, int IDLang, bool Disable)
        {
            try
            {
                return aDatabaseDA.|@Table@|.Where(p => p.Status == Status).Where(z => z.IDLang == IDLang).Where(z => z.Disable == Disable).ToList();
            }
            catch (Exception ex)
            {
                throw new Exception(String.Format("|@Table@|BO.Sel_ByStatus:{0}", ex.Message));
        ...
        public List<|@Table@|> Sel_ByType_ByStatus_ByIDLang(int Type, int Status, int IDLang)
        {
            try
            {
                return aDatabaseDA.|@Table@|.Where(c => c.Type == Type).Where(p => p.Status == Status).Where(z => z.IDLang == IDLang).ToList();
            }
            catch (Exception ex)
            {
                throw new Exception(String.Format("|@Table@|BO.Sel_ByType_ByStatus_ByIDLang:{0}", ex.Message));
            }
        }
        public List<|@Table@|> Sel_ByType_ByStatus_ByIDLang_ByDisable(int Type, int Status, int IDLang, bool Disable)
        {
            try
            {
                return aDatabaseDA.|@Table@|.Where(c => c.Type == Type).Where(p => p.Status == Status).Where(z => z.IDLang == IDLang).Where(z => z.Disable == Disable).ToList();
            }
            catch (Exception ex)
            {
                throw new Exception(String.Format("|@Table@|BO.Sel_ByType_ByStatus_ByIDLang:{0}", ex.Message));
        ...
        public List<|@Table@|> Sel_ByCode(string Code)
        {
            try
            {
                return aDatabaseDA.|@Table@|.Where(p => p.Code == Code).ToList();
            }
            catch (Exception ex)
            {
                throw new Exception(String.Format("|@Table@|BO.Sel_ByCode: {0}", ex.Message));
            }
        }
        public List<|@Table@|> Sel_ByCode(string Code, bool Disable)
        {
            try
            {
                return aDatabaseDA.|@Table@|.Where(p => p.Code == Code).Where(p => p.Disable == Disable).ToList();
            }
            catch (Exception ex)
            {
                throw new Exception(String.Format("|@Table@|BO.Sel_ByCode: {0}", ex.Message));
        ...
        public int Del()
        {
            ...
            {
                throw new Exception(String.Format("|@Table@|BO.Del_ByCode: {0}", ex.Message));
            }
        }
       ...
       [@Loop looptype='Column' by='KeyColumn'  @]
		...
		public int Del_By|@Column@|(Int32 |@Column@|)
        {
            try
            {
                ...
                {
                    throw new Exception(String.Format("|@Table@|BO.Del: {0}", "Không tìm thấy |@Table@| có |@Column@| = " + |@Column@|));
                }
        ...
        public int Upd_Status_ByCode(string Code, int NewStatus) 
        {
            try
            {
                List<|@Table@|> aList|@Table@| = aDatabaseDA.|@Table@|.Where(b => b.Code==Code).ToList();
                if (aList|@Table@|.Count > 0)
                {
        ...
        public int Upd_Status_ByCode_ByLang(string Code, int NewStatus, int IDLang) 
        {
            try
            {
                List<|@Table@|> aList|@Table@| = aDatabaseDA.|@Table@|.Where(b => b.Code == Code).Where(z=>z.IDLang==IDLang).ToList();
                if (aList|@Table@|.Count > 0)
                {
                    aList|@Table@|[0].Status = NewStatus;
                    aDatabaseDA.|@Table@|.AddOrUpdate(aList|@Table@|[0]);
        ...
        public int Del_ByCode(string Code)
        {
            try
            {
                List<|@Table@|> aTemp = aDatabaseDA.|@Table@|.Where(p => p.Code == Code).ToList();
                if (aTemp.Count > 0)
                {
                    aDatabaseDA.|@Table@|.RemoveRange(aTemp);
                    return aDatabaseDA.SaveChanges();
        ...
        public int Del_ByCode_ByIDLang(string Code, int IDLang)
        {
            try
            {
                List<|@Table@|> aTemp = aDatabaseDA.|@Table@|.Where(p => p.Code == Code).Where(p => p.IDLang == IDLang).ToList();
                if (aTemp.Count >0)
                {
                    aDatabaseDA.|@Table@|.Remove(aTemp[0]);
                    return aDatabaseDA.SaveChanges();
            ...
            {
                throw new Exception(String.Format("|@Table@|BO.Del_ByCode_ByIDLang: {0}", ex.Message));
            }
        }
        public int Del_By_ListCode(List<string> aListCode)
        {
            try
            {
                List<|@Table@|> aTemp = aDatabaseDA.|@Table@|.Where(p => aListCode.Contains(p.Code)).ToList();
                if (aTemp.Count > 0)
                {
                    aDatabaseDA.|@Table@|.RemoveRange(aTemp);
                    return aDatabaseDA.SaveChanges();
        ...
        //NgocBM
        public |@Table@| Sel_ByCode_ByIDLang(string Code, int IDLang)
        {
            try
            {
                ...
                aList|@Table@| = aDatabaseDA.|@Table@|.Where(a => a.Code == Code).Where(a => a.IDLang == IDLang).ToList();
                if (aList|@Table@|.Count > 0)
                {
                    return aList|@Table@|[0];
                }
                else
                    return null;
            }
            ...
            {
                throw new Exception(String.Format("|@Table@|BO.SelectList|@Table@|_ByCode_ByCategoryL1: {0}", ex.Message));
            }
        }
        public |@Table@| Sel_ByCode_ByIDLang(string Code, int IDLang, bool Disable)
        {
            try
            {
                ...
                aList|@Table@| = aDatabaseDA.|@Table@|.Where(a => a.Code == Code).Where(a => a.IDLang == IDLang).Where(a => a.Disable == Disable).ToList();
                if (aList|@Table@|.Count > 0)
                {
                    return aList|@Table@|[0];
                }
